...
def load_locations(mech, mech_model):
    ...
    for (ssw_code, armour) in mech.armour.armour.items():
        loc_code = translate.locations_all[ssw_code]
...
def load_equipment(mech, mech_id, equipment, mechlocations):
    ...
    if eq_form.is_valid():
        ...
        for ssw_loc, mount in equipment.mountings.items():
            loc_code = translate.locations_all[ssw_loc]
...
def load_mech(sswfile):
    try:
        ...
        load_equipment(mech, mech_object.id, mech.engine, locations)
        load_equipment(mech, mech_object.id, mech.armour, locations)        
