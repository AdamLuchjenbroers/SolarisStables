...
class RefitUploadNotLoggedInTest(StableTestMixin, TestCase):
    def setUp(self):
        ...
        self.stablemech = self.addMech(self.stable, mech_name='Wolverine', mech_code='WVR-7D')
...
class RefitIncorrectFormTests(StableTestMixin, TestCase):
    def setUp(self):
        ...
        self.refit_url  = reverse('upload_refit_mech', kwargs={'smw_id' : self.stablemechweek.id})
    ...
    def test_wrong_mechfile(self):
        filename = 'data/test-mechs/SimpleLoadingTest SLT-1.ssw'
    ...
    def test_existing_config(self):
        filename = 'data/test-mechs/TestWolverine WVR-6K.ssw'
        
        with open(filename, 'rb') as ssw_data:
            response = self.client.post(self.refit_url, {'ssw_file' : ssw_data})
        
        self.assertEqual(response.status_code, 400, 'Form with existing mech design returns incorrect status code (HTTP %s)' % response.status_code)

    def test_wrong_tonnage(self):
        filename = 'data/test-mechs/TestWolverine WVR-2TST Low Weight.ssw'
        
        with open(filename, 'rb') as ssw_data:
            response = self.client.post(self.refit_url, {'ssw_file' : ssw_data})
        
        self.assertEqual(response.status_code, 400, 'Form with incorrect design tonnage returns incorrect status code (HTTP %s)' % response.status_code)
        
class RefitUploadTest(StableTestMixin, TestCase):
    ssw_filename = 'TestWolverine WVR-1TST.ssw'
